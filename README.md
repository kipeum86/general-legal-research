...
| `library/` | Attorney (manual upload) | Your own legal opinions, case law, papers — treated as **Grade A** sources |
...
**Auto-conversion:** Run `python3 scripts/library-ingest.py` to convert all PDF/DOCX/PPTX files in `library/` to searchable Markdown in `knowledge/library-converted/`. The index is updated automatically.

```text
library/
|-- _index.md          # auto-generated by library-ingest.py
...
|   |-- library-ingest.py                # convert library/ PDFs to Markdown

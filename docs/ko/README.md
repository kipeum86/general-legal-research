...
| **3** | Source Collection | 메타데이터가 포함된 원문 소스 수집 (한국법은 korean-law MCP Server + 열린법령 API 우선; PDF/DOCX는 MarkItDown으로 자동 변환) |
...
| 모드 | 가능한 기능 | 제한 |
|:-----|:-----------|:-----|
| **Local-only** | 열린법령 API로 한국 법령·판례·해석례 조회, 허용된 법률 포털 직접 URL fetch, skill dispatch, 결과물 생성, `library/` PDF 인제스트 (`markitdown` Python API) | 키워드 검색 (`tavily` / `brave`), korean-law MCP 도구 |

...
| **3** | Source Collection | 메타데이터가 포함된 원문 소스 수집 (PDF/DOCX는 MarkItDown으로 자동 변환) |
...
| `web-researcher` | 3 | 공식 포털에서 소스 수집 |
...
| 모드 | 가능한 기능 | 제한 |
|:-----|:-----------|:-----|
| **Local-only** | 허용된 법률 포털 직접 URL fetch, skill dispatch, 결과물 생성, `library/` PDF 인제스트 (`markitdown` Python API) | 키워드 검색 (`tavily` / `brave`) |
